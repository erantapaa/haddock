...
var current_span;    // currently highlighted span
...
function handle_click(e) {
  ...
  if (s) {
    current_span = s
    highlight_span(s)
    console.log("type at line", found, "" + lpos + "-" + rpos, s[4])
    tooltip_display("type: " + s[4], elt)
  } else {
    console.log("no span found")
  }
}

function grow_current_span() {
  if (!current_span) return
  var s = grow_span(current_span)
  if (s) {
    current_span = s
    highlight_span(s)
    tooltip_display("type: " + s[4])
...
function initialize() {
  ...
  var src_url = "spans/spans-" + modname + ".js"
